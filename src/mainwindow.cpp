...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent)
{
    ...
    statusbar->addPermanentWidget(elapsed_timer);
    getLogicalDrives();
    status = STATUS_IDLE;
    progressbar->reset();
    ...
    connect(this->cboxHashType, &QComboBox::currentIndexChanged, this, &MainWindow::on_cboxHashType_IdxChg);
    updateHashControls();
...
{
    if (status == STATUS_IDLE || status == STATUS_CANCELED)
    {
        event->accept();
        return;
    }
    ...
    if (status == STATUS_READING)
        msg = tr("Exiting now will result in a corrupt image file.\nAre you sure?");
    else if (status == STATUS_WRITING)
        msg = tr("Exiting now will result in a corrupt disk.\nAre you sure?");
    else if (status == STATUS_VERIFYING)
        msg = tr("Exiting now will cancel verifying.\nAre you sure?");
    else
    { event->accept(); return; }

    if (QMessageBox::warning(this, tr("Exit?"), msg,
...
void MainWindow::generateHash(char *filename, int hashish)
{
    ...
    if (!file.open(QFile::ReadOnly))
    {
        ...
        hashLabel->setText(tr("Error opening file"));
        return;
    }
...
void MainWindow::on_bWrite_clicked()
{
    ...
    if (!leFile->text().isEmpty())
    {
                ...
                fileinfo.isReadable() && (fileinfo.size() > 0) )
        {
            ...
            hFile = getHandleOnFile(leFile->text().toStdWString().c_str(), GENERIC_READ);
            if (hFile == INVALID_HANDLE_VALUE)
            {
                removeLockOnVolume(hVolume);
                CloseHandle(hVolume);
            ...
            if (numsectors > availablesectors)
            {
                ...
                if(QMessageBox::warning(this, tr("Not enough available space!"),
                                        msgStr, QMessageBox::Ok, QMessageBox::Cancel) == QMessageBox::Ok)
                {
            ...
            int progressMax = (numsectors > (unsigned long long)INT_MAX) ? INT_MAX : (numsectors == 0ul ? 100 : (int)numsectors);
            progressbar->setRange(0, progressMax);
            lasti = 0ul;
            update_timer.start();
            elapsed_timer->start();
            for (i = 0ul; i < numsectors && status == STATUS_WRITING; i += 1024ul)
            {
                ...
                progressbar->setValue((numsectors > (unsigned long long)INT_MAX) ? (int)((double)i / numsectors * INT_MAX) : (int)i);
                QCoreApplication::processEvents();
            }
            removeLockOnVolume(hVolume);
            CloseHandle(hRawDisk);
...
void MainWindow::on_bRead_clicked()
{
    ...
    if (!leFile->text().isEmpty())
    {
        ...
        hFile = getHandleOnFile(myFile.toStdWString().c_str(), GENERIC_WRITE);
        if (hFile == INVALID_HANDLE_VALUE)
        {
            removeLockOnVolume(hVolume);
            CloseHandle(hVolume);
        ...
        if(partitionCheckBox->isChecked())
        {
            ...
            if (sectorData == nullptr)
            {
                QMessageBox::critical(this, tr("Read Error"), tr("Failed to read partition table."));
                removeLockOnVolume(hVolume);
                CloseHandle(hRawDisk);
                CloseHandle(hFile);
                CloseHandle(hVolume);
        ...
        QString drivePath = QFileInfo(myFile).absolutePath();
        if (!spaceAvailable(drivePath.toLocal8Bit().data(), spaceneeded))
        {
        ...
        {
            int progressMax = (numsectors > (unsigned long long)INT_MAX) ? INT_MAX : (int)numsectors;
            progressbar->setRange(0, progressMax);
        }
        lasti = 0ul;
        update_timer.start();
...
void MainWindow::getLogicalDrives()
{
    ...
    while (driveMask != 0)
    {
        if (driveMask & 1)
        {
            ...
            {
                cboxDevice->addItem(QString("[%1:\\]").arg(drivename[4]), (quint64)pID);
            }
        }
        driveMask >>= 1;
            ...
            if (lpdb -> dbch_devicetype == DBT_DEVTYP_VOLUME)
            {
                ...
                if(!(lpdbv->dbcv_flags & DBTF_NET))
                {
                    ...
                    if (cboxDevice->findText(qs) == -1)
                    {
                        ...
                        {
                            cboxDevice->addItem(qs, (quint64)pID);
                            setReadWriteButtonState();
                        }
                    }
                }
